...
import { Plus, Trash2, PackageMinus, Eye, AlertTriangle } from 'lucide-react'
...
export default function StockOut() {
  ...
  const create = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('Stock OUT recorded successfully'); setModal(null); setForm(EMPTY_FORM)
    },
    onError: e => toast.error(e.response?.data?.message || 'Error'),
  })

  const getAvail = (productId) => stocks?.data?.find(s => String(s.ProductId) === String(productId))?.quantity ?? null
  ...
  return (
    <div className="px-6 py-6">
      <PageHeader title="Stock OUT" subtitle={`${data?.pagination?.total ?? 0} transactions`}
        action={<button onClick={() => { setForm(EMPTY_FORM); setItem(EMPTY_ITEM); setModal({ mode: 'create' }) }} className="btn-primary"><PackageMinus size={14} />New Stock OUT</button>}
      />
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select className="select" value={form.warehouseId} onChange={e => setForm(f => ({ ...f, warehouseId: e.target.value, items: [] }))} required>
          ...
          <div className="border border-slate-200 rounded-lg overflow-hidden">
            ...
            <div className="p-3 bg-white flex gap-2 border-b border-slate-100">
              <select className="select flex-1" value={item.productId} onChange={e => setItem(i => ({ ...i, productId: e.target.value }))} disabled={!form.warehouseId}>
              ...
              <input className="input w-24" type="number" min="1" placeholder="Qty" value={item.quantity} onChange={e => setItem(i => ({ ...i, quantity: e.target.value }))} disabled={!form.warehouseId} />
              <button type="button" onClick={addItem} className="btn-primary px-3" disabled={!form.warehouseId}><Plus size={14} /></button>
            </div>
            ...
            {!form.items.length ? (
              ...
              <table className="w-full bg-white text-sm">
                <thead><tr className="bg-slate-50 border-b border-slate-100">
                  <th className="th py-2">Product</th><th className="th py-2 text-right">Qty</th><th className="th py-2 text-right">Available</th><th className="th py-2 w-10"></th>
                </tr></thead>
                <tbody>
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button type="submit" disabled={create.isPending} className="btn-primary flex-1 justify-center">{create.isPending ? 'Submitting…' : 'Submit Stock OUT'}</button>
          </div>
        </form>
      </Modal>
