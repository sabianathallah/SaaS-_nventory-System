...
export default function Suppliers() {
  ...
  const save = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries(['suppliers']); toast.success(modal.data ? 'Supplier updated' : 'Supplier created'); setModal(null) },
    onError: e => toast.error(e.response?.data?.message || 'Error'),
  })
  const del = useMutation({
    mutationFn: id => suppliersApi.remove(id),
    onSuccess: () => { qc.invalidateQueries(['suppliers']); toast.success('Supplier deleted'); setModal(null) },
    onError: e => toast.error(e.response?.data?.message || 'Error'),
  })
  ...
  return (
    <div className="px-6 py-6">
      ...
      <Modal open={['create','edit'].includes(modal?.mode)} onClose={() => setModal(null)}
        ...
        <form onSubmit={e => { e.preventDefault(); save.mutate(form) }} className="space-y-4">
          <div>
            ...
            <input className="input" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} required autoFocus placeholder="e.g. PT Supplier Indonesia" />
          </div>
          <div>
      ...
      <Modal open={modal?.mode === 'delete'} onClose={() => setModal(null)} title="Delete Supplier" size="sm">
        <p className="text-sm text-slate-600 mb-5">Delete <span className="font-semibold text-slate-800">"{modal?.data?.name}"</span>?</p>
