  ...
  static async get(req, res, next) {
    try {
      const user = await User.findByPk(req.user.id, {
        attributes: ['id', 'name', 'email', 'role', 'companyId', 'avatar', 'divisi', 'divisis', 'isActive'],
      });
  ...
  static async update(req, res, next) {
    try {
      ...
      if (req.body.divisis !== undefined) {
        const divisis = [...new Set((Array.isArray(req.body.divisis) ? req.body.divisis : [req.body.divisis])
          .map(d => String(d).trim()).filter(Boolean))];
        user.divisis = divisis;
        user.divisi = divisis[0] || null;
      } else if (req.body.divisi !== undefined) {
        user.divisi = req.body.divisi?.trim() || null;
      }
      ...
      await user.save();
      res.json({ id: user.id, name: user.name, email: user.email, role: user.role, companyId: user.companyId, avatar: user.avatar, divisi: user.divisi, divisis: user.divisis });
    } catch (err) { next(err); }
  }
